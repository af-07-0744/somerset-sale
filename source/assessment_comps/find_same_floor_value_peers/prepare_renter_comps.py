...
DEFAULT_INPUT_CSV = Path(
    "source/assessment_comps/match_same_unit_across_buildings/data/generated/open_calgary_assessment_comps.csv"
)
DEFAULT_OUTPUT_CSV = Path(
    "source/assessment_comps/find_same_floor_value_peers/data/generated/renter_comps_table.csv"
)
DEFAULT_OUTPUT_RST = Path("source/assessment_comps/find_same_floor_value_peers/renter_comps_generated.rst")
DEFAULT_STRETCH_OUTPUT_CSV = Path(
    "source/assessment_comps/find_same_floor_value_peers/data/generated/renter_comps_stretch_table.csv"
)
DEFAULT_STRETCH_OUTPUT_RST = Path(
    "source/assessment_comps/find_same_floor_value_peers/renter_comps_stretch_generated.rst"
)
...
def _build_table_rst(
    ...
    if included_rows > 0:
        ...
        lines.extend(
            [
                "No rows met the current filter criteria for this view.",
                "",
            ]
        )
    lines.extend(
        [
            "Source details and provenance IDs are listed in "
            ":doc:`/fair_market_value/write_fmv_justification/appendix_sources`.",
            "",
        ]
    )
    return "\n".join(lines)
