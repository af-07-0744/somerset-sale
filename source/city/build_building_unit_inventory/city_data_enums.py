...
DEFAULT_OUTPUT_RST = Path("source/city/build_building_unit_inventory/city_data_enum_dictionary.rst")
DEFAULT_RST_TABLE_DIR = Path("source/city/build_building_unit_inventory/city_data/_tables/enums")
DEFAULT_DOWNLOAD_DIR = Path("source/city/build_building_unit_inventory/city_data/_downloads/enums")
DEFAULT_RST_TITLE = "City Data Enumeration Dictionary"
DEFAULT_RUN_META_JSON = Path(
...
DATA_FILE_TABLE_FIELDNAMES = ["Name", "Column", "File", "Type", "Rows", "Fields", "Download"]
FILE_FIELD_PROV_FIELDNAMES = ["File", "Field", "Origin"]
REQUEST_INDEX_FIELDNAMES = ["Request", "Column", "Stage", "Where", "URL"]
REQUEST_PARAM_FIELDNAMES = ["Name", "Value"]
...
RUN_META_FIELD_ORIGINS = {
    ...
    "subject_address_matched": "Best-matching subject address selected from subject lookup rows.",
    "fetch_scope": "Set by fetch script mode (subject_street).",
    "subject_lookup_where_clauses": "Subject lookup WHERE clauses attempted before street expansion.",
    "subject_lookup_queries": "Subject lookup $q strings attempted before street expansion.",
    "subject_lookup_query_urls": "Recorded query URLs for subject lookup attempts.",
    "street_portion": "Derived from matched subject-row address or explicit street override.",
    ...
    "where_clauses": "Generated by fetch script for derived street matching.",
...
def _fields_for_file(path: Path) -> list[str]:
    if not path.exists():
        return []
    if path.suffix.lower() == ".csv":
        return _csv_headers(path)
    if path.suffix.lower() == ".json":
        return _detect_json_fields(path)
    return []
...
def _where_like_tokens(where_clause: str) -> list[str]:
    ...
    return _unique_preserve([token for token in tokens if token])
...
def _build_request_entries(run_meta: dict[str, Any]) -> list[dict[str, Any]]:
    stage_specs = [
        {
    ...
    for spec in stage_specs:
        raw_urls = run_meta.get(spec["url_key"], [])
        query_urls = [str(url) for url in raw_urls] if isinstance(raw_urls, list) else []
        where_order = run_meta.get(spec["where_key"], [])
        ordered_where = [str(item) for item in where_order] if isinstance(where_order, list) else []
        ...
        for index, url in enumerate(query_urls, start=1):
            ...
            offset_text = params.get("$offset", "")
            try:
                offset_num = int(offset_text)
            except ValueError:
                offset_num = 0
            entries.append(
                {
                    ...
                    "dataset_id": parsed.path.split("/")[-1].replace(".json", ""),
                    "stage": spec["stage"],
                    "prefix": spec["prefix"],
                }
            )
        ...
        for where_clause in ordered_where:
            matches = [
                item
                for item in entries
                if item["where_clause"] == where_clause and item["original_index"] not in used
            ]
            matches.sort(key=lambda item: (item["offset"], item["original_index"]))
            for item in matches:
                used.add(item["original_index"])
                ordered.append(item)

        for item in entries:
            if item["original_index"] in used:
                continue
            ordered.append(item)

        stage_counts: dict[str, int] = {}
        for index, item in enumerate(ordered, start=1):
            base = _request_label_from_where(item["where_clause"], spec["stage"], index)
            stage_counts[base] = stage_counts.get(base, 0) + 1
            suffix = stage_counts[base]
            item["name"] = f"{base} ({suffix})" if suffix > 1 else base

        stage_entries.extend(ordered)

    return stage_entries


def _like_pattern(where_clause: str) -> str:
    match = re.search(r"(?i)like\s+'%(.+?)%'", where_clause)
    if not match:
        return ""
    return _normalize_text(match.group(1)).upper()


def _fields_by_request_from_rows(
    *,
    ...
    fallback = {str(item.get("name", "")): set(input_fields) for item in request_entries}
    if (not request_entries) or (not raw_rows_json.exists()):
        return fallback

    try:
        payload = _read_json(raw_rows_json)
    except Exception:
        return fallback
    ...
    for item in request_entries:
        request_name = str(item.get("name", ""))
        if not request_name:
            continue
        where_clause = str(item.get("where_clause", ""))
        needle = _like_pattern(where_clause)
        matched_rows = rows
        if needle:
            matched_rows = [
                row
                for row in rows
                if needle in _normalize_text(row.get(address_field, "")).upper()
            ]
        if not matched_rows:
            mapping[request_name] = set(input_fields)
            continue
        fields: set[str] = set()
        for row in matched_rows:
            fields.update(str(key) for key in row.keys())
        mapping[request_name] = fields or set(input_fields)
    return mapping


def _short_file_column_name(index: int, label: str) -> str:
...
def _matrix_rows(
    *,
    ...
    fieldnames = ["Field", *column_labels]
    rows: list[dict[str, str]] = []
    for row_label in row_labels:
        row_data: dict[str, str] = {"Field": row_label}
        for column_label in column_labels:
            row_data[column_label] = "✓" if row_label in membership.get(column_label, set()) else ""
...
def _render_enum_rst(
    *,
    ...
    enum_table_fields = ["Value", "Count", "%", "% Present", "Meaning", "How Resolved*"]
    enum_table_path_by_field: dict[str, Path] = {}
    for field in selected_fields:
        ...
        _write_csv(table_path, enum_table_fields, table_rows)
        enum_table_path_by_field[field] = table_path

    data_file_specs: list[tuple[str, Path, str]] = [
        ("Input Flat CSV", input_csv, "csv"),
        ("Enumeration Dictionary CSV", output_csv, "csv"),
    ]
    if run_meta_json.exists():
        data_file_specs.append(("Fetch Run Metadata JSON", run_meta_json, "json"))
    for field in selected_fields:
        path = enum_table_path_by_field.get(field)
        if path is None:
            continue
        data_file_specs.append((f"Enum Table CSV ({field})", path, "csv"))

    data_file_rows: list[dict[str, str]] = []
    data_file_column_membership: dict[str, set[str]] = {}
    download_lines: list[str] = []
    for index, (label, path, file_type) in enumerate(data_file_specs, start=1):
        copied = _copy_download_file(path, download_dir)
        if copied is None:
            continue
        rows_count, field_count = _summarize_file(path)
        column_name = _short_file_column_name(index, label)
        ...
        data_file_column_membership[column_name] = set(_fields_for_file(path))
    ...
    lines.append(f"- Fields documented: ``{len(selected_fields)}``")
    lines.append("- Last-run fetch/request metadata: :doc:`city_data_fetch_metadata`")
    lines.append("")
    ...
    for field in selected_fields:
        ...
        lines.append("")
        table_path = enum_table_path_by_field.get(field)
