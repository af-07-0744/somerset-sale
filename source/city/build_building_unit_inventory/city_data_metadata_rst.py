...
from city.fetch_subject_street_assessments.fetch_city_data import DEFAULT_OUTPUT_META_JSON
from city.build_subject_unit_profile.city_data_inventory import DEFAULT_SUMMARY_JSON


DEFAULT_OUTPUT_RST = Path("source/city/build_building_unit_inventory/city_data_fetch_metadata.rst")
DEFAULT_TABLE_DIR = Path("source/city/build_building_unit_inventory/city_data/_tables/meta")
...
DEFAULT_FLAT_CSV = Path(
    "source/city/fetch_subject_street_assessments/data/generated/open_calgary_somervale_raw_rows_flat.csv"
)
PRICE_FIELDS = {
    "assessed_value",
    "re_assessed_value",
...
def render_city_data_metadata_rst(
    *,
    ...
    lines.append("   }")
    lines.append(
        "   file \"source/city/fetch_subject_street_assessments/data/generated/"
        "open_calgary_somervale_raw_rows.json\" as raw_json"
    )
    lines.append(
        "   file \"source/city/fetch_subject_street_assessments/data/generated/"
        "open_calgary_somervale_raw_rows_flat.csv\" as flat_csv"
    )
    lines.append(
        "   file \"source/city/fetch_subject_street_assessments/data/generated/"
        "open_calgary_somervale_raw_\\nfield_profile.csv\" as field_profile"
    )
    lines.append(
        "   file \"source/city/build_subject_unit_profile/data/generated/"
        "open_calgary_somervale_inventory_\\nsummary.json\" as inventory_summary"
    )
    lines.append(
        "   file \"source/city/build_building_unit_inventory/data/generated/"
        "open_calgary_street_requested_\\nfield_dictionary.csv\" as enum_dictionary"
    )
    lines.append("   file \"source/city/build_building_unit_inventory/city_data_enum_dictionary.rst\" as enum_rst")
    lines.append(
        "   file \"source/city/build_building_unit_inventory/city_data_index.rst\\n"
        "source/city/build_building_unit_inventory/city_data/\\nbuilding_*.rst\" as city_pages"
    )
    lines.append("   file \"source/city/build_building_unit_inventory/city_data_fetch_metadata.rst\" as metadata_rst")
    lines.append("")
    lines.append("   query_1 --> query_2 : 1) street_portion")
    lines.append("")
    lines.append("   query_2 --> raw_json")
    lines.append("   raw_json -down-> flat_csv : 2) flattened rows")
    lines.append("   raw_json -left-> field_profile : 3) field profile")
    lines.append("")
    lines.append("   flat_csv -down-> inventory_summary : 4) inventory output")
    lines.append("   flat_csv -right-> city_pages : 5) page rows")
    lines.append("   raw_json -right-> city_pages : 6) provenance")
    lines.append("")
    lines.append("   inventory_summary -down-> metadata_rst : 7) inventory input")
    lines.append("   flat_csv -right-> enum_dictionary : 8) enum dictionary")
    lines.append("   enum_dictionary -down-> enum_rst : 9) dictionary source")
    lines.append("")
    lines.append("   field_profile -[hidden]down-> inventory_summary")
    lines.append("   enum_rst -[hidden]down-> city_pages")
    lines.append("   @enduml")
    lines.append("")
